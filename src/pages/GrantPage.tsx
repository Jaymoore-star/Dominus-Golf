      ...
      <div ref={heroRef} className="relative min-h-[90vh] flex flex-col pt-20">
        ...
        >
          ...
          >
            Dominus Golf
            <br />
            <span className="text-accent">Development Grant</span>
          </motion.h1>
          ...
          >
            <div className="flex flex-wrap items-center justify-center gap-0 border border-border bg-secondary">
              {STATS.map((stat, i) => (
                <div
                  key={stat.label}
                ...
                >
                  <span className="text-lg sm:text-xl font-bold font-serif text-foreground tracking-tight">
                    {stat.value}
                  </span>
                  <span className="ml-2.5 text-[11px] sm:text-xs uppercase tracking-wider text-muted-foreground font-sans font-medium">
                    {stat.label}
                  </span>
                </div>
              ))}
          ...
          <div className="grid lg:grid-cols-5 gap-12 lg:gap-16">
            ...
            <FadeUpSection className="lg:col-span-2" delay={0.15}>
              <div className="border border-border bg-secondary p-6 sm:p-7">
                <h3 className="font-serif text-lg font-semibold text-foreground mb-5 tracking-tight">
                  Grant Details
                </h3>
                  ...
                  {DETAILS.map((d, i) => (
                    <div key={i} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                      <dt className="flex items-center gap-2.5 text-xs uppercase tracking-wider text-muted-foreground font-sans font-medium">
                        <d.icon className="w-3.5 h-3.5 text-accent" />
                        {d.label}
                      </dt>
                      <dd className="text-sm text-foreground font-sans font-medium text-right">
                        {d.value}
                      </dd>
                    </div>
                  ))}
            ...
            {STEPS.map((step, i) => (
              <FadeUpSection key={step.num} delay={i * 0.1} className="bg-background">
                <div className="p-6 sm:p-7 lg:p-8 h-full flex flex-col">
                  <span className="text-accent text-xs tracking-[0.25em] uppercase font-sans font-semibold mb-4">
                    Step {step.num}
                  </span>
                  <h3 className="font-serif text-xl font-semibold text-foreground mb-3 tracking-tight leading-tight">
                    {step.title}
                  </h3>
                  <p className="text-sm text-muted-foreground leading-relaxed mt-auto font-sans">
                    {step.body}
                  </p>
                </div>
              </FadeUpSection>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {ELIGIBILITY.map((card, i) => (
              <FadeUpSection
                key={card.title}
                delay={i * 0.1}
                className="border border-border bg-secondary p-7 sm:p-8 flex flex-col items-center text-center"
              >
          ...
          <FadeUpSection delay={0.1}>
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="name" className="block text-xs uppercase tracking-widest text-muted-foreground font-sans font-semibold mb-2">
                    Full Name *
                  </label>
                  <input
                    id="name"
                    ...
                    required
                    disabled={submitting}
                    className="w-full bg-background border border-border px-4 py-3 text-sm text-foreground font-sans
                      placeholder:text-muted-foreground/50 focus:outline-none focus:border-accent/60 focus:ring-1 focus:ring-accent/20 transition-colors
                      disabled:opacity-50"
                  />
                </div>
                <div>
                  <label htmlFor="email" className="block text-xs uppercase tracking-widest text-muted-foreground font-sans font-semibold mb-2">
                    Email Address *
                  </label>
                  <input
                    id="email"
              ...
              {FORM_FIELDS.map((field) => (
                <div key={field.id}>
                  ...
                  />
                  <p className="text-right text-[10px] text-muted-foreground/50 mt-1 font-sans">
              ...
              <div className="flex flex-col items-center gap-3 pt-4">
                <button
                  ...
                  className="inline-flex items-center gap-2.5 px-10 py-4 text-sm font-semibold tracking-wider uppercase
                    btn-gold active:scale-[0.98] transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {submitting ? (
                    <>
